...
ASSETS = {
    "XAUUSD": {"ticker":"GC=F",    "name":"Gold",    "emoji":"🥇","sl_pts":50, "tp_pts":80, "dec":2},
    "BTCUSD": {"ticker":"BTC-USD", "name":"Bitcoin", "emoji":"₿", "sl_pts":400,"tp_pts":600,"dec":1},
}
...
def fetch(ticker):
    try:
        df=yf.download(ticker,period=DATA_PERIOD,interval=CANDLE_TF,progress=False,auto_adjust=True)
        if df.empty: return df
        if isinstance(df.columns,pd.MultiIndex): df.columns=df.columns.get_level_values(0)
        df.dropna(inplace=True); return df
    except Exception as e:
        log.error(f"fetch({ticker}): {e}"); return pd.DataFrame()
...
def calc_fib(df,window=80):
    w=df.tail(window); hi,lo=float(w["High"].max()),float(w["Low"].min()); d=hi-lo
...
def fair_value_gaps(df,window=50):
    w=df.tail(window).reset_index(drop=True)
...
def bos_choch(df,window=40):
    w=df.tail(window); half=window//2
    p_hi=float(w.iloc[:half]["High"].max()); p_lo=float(w.iloc[:half]["Low"].min())
    last=float(w["Close"].iloc[-1])
    if last>p_hi: return "BULL_BOS"
    if last<p_lo: return "BEAR_BOS"
    return "NONE"

def market_structure(df,window=60):
    w=df.tail(window); mid=window//2; h,l=w["High"].values,w["Low"].values
...
def ict_zone(fib,price): return "PREMIUM" if price>fib["0.5"] else "DISCOUNT"
...
def liquidity_sweep(df,window=30):
...
def score_and_signal(asset_key,df):
    if len(df)<80: return None
    asset=ASSETS[asset_key]; dec=asset["dec"]; close=float(df["Close"].iloc[-1])
    ema9=float(ema(df["Close"],9).iloc[-1]); ema21=float(ema(df["Close"],21).iloc[-1])
    vwap=calc_vwap(df); rsi=calc_rsi(df["Close"]); fib=calc_fib(df)
    b_ob,r_ob=order_blocks(df); b_fvg,r_fvg=fair_value_gaps(df)
    bos=bos_choch(df); structure=market_structure(df)
    zone=ict_zone(fib,close); candle_pat=engulfing(df); liq=liquidity_sweep(df)
    tol=fib["diff"]*0.012
    bs=0; rs=0; bw=[]; rw=[]
    ...
    for name,val in [("61.8%",fib["0.618"]),("70.5%",fib["0.705"]),("78.6%",fib["0.786"])]:
        if abs(close-val)<=tol and zone=="DISCOUNT": bs+=2; bw.append(f"Fib {name} support @ {val:.{dec}f} Discount"); break
    for name,val in [("23.6%",fib["0.236"]),("38.2%",fib["0.382"]),("50.0%",fib["0.5"])]:
        if abs(close-val)<=tol and zone=="PREMIUM":  rs+=2; rw.append(f"Fib {name} resistance @ {val:.{dec}f} Premium"); break
    if b_ob:
    ...
    if liq=="BULL_SWEEP": bs+=1; bw.append("ICT liquidity sweep buy-side")
    elif liq=="BEAR_SWEEP":rs+=1; rw.append("ICT liquidity sweep sell-side")
    ...
    return {"direction":direction,"entry":entry,"sl":sl,"tp":tp,
            "ema9":round(ema9,dec),"ema21":round(ema21,dec),"vwap":round(vwap,dec),"rsi":round(rsi,1),
            "fib_236":round(fib["0.236"],dec),"fib_382":round(fib["0.382"],dec),
            "fib_50":round(fib["0.5"],dec),"fib_618":round(fib["0.618"],dec),"fib_786":round(fib["0.786"],dec),
            "fib_hi":round(fib["high"],dec),"fib_lo":round(fib["low"],dec),
            "structure":structure,"zone":zone,"bos":bos,"bull_s":bs,"bear_s":rs,"score":score,"reasons":reasons}
...
def main():
    log.info(f"AURUM AI starting — running for {RUN_DURATION_SEC}s")
    start=time.time()
    while (time.time()-start)<RUN_DURATION_SEC:
        for asset_key,asset in ASSETS.items():
            try:
                df=fetch(asset["ticker"])
                if df.empty: log.warning(f"{asset_key}: no data"); continue
                ...
                if sig_id==_last_sig_id[asset_key]: log.info(f"  {asset_key}: duplicate, skip"); continue
                _last_sig_id[asset_key]=sig_id
                msg=fmt_msg(asset_key,sig)
                if tg(msg): log.info(f"  SIGNAL SENT {asset_key} {sig['direction']} @ {sig['entry']} score={sig['score']}/14")
            except Exception as e:
    ...
    log.info("Done — GitHub Actions will restart in 5 min")
