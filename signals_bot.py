...
def fetch(ticker: str) -> pd.DataFrame:
    ...
    fallbacks = {
        ...
        "BTC-USD": ["BTC-USD", "BTCUSDT=X"],
    }
    tickers_to_try = fallbacks.get(ticker, [ticker])

    for t in tickers_to_try:
        try:
            obj = yf.Ticker(t)
            df  = obj.history(period=DATA_PERIOD, interval=CANDLE_TF)
            if df.empty:
                log.warning(f"fetch: {t} returned empty, trying next...")
                continue
            # Rename columns to standard names if needed
            df.columns = [c.capitalize() for c in df.columns]
            df.dropna(inplace=True)
            log.info(f"fetch: got {len(df)} candles from {t}")
            return df
        except Exception as e:
            log.warning(f"fetch: {t} failed — {e}")
            time.sleep(2)
            continue

    log.error(f"fetch: all tickers failed for {ticker}")
    return pd.DataFrame()
